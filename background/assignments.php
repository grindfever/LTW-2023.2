...
if(isset($_SESSION['username']) && $_SESSION['usertype'] != 'client'){
 if((isset($_SESSION['admin_type']) && $_SESSION['admin_type'] == 'main admin') || $_SESSION['user_id'] == $agent_id || $_SESSION['user_id'] == $admin_id ){
  if(isset($_GET['ticket_id'])){
   ...
   $stmt->execute();
   ob_clean();
   header('Location:../staff/ticket-inbox');
   echo '<script>alert("Ticket has been assigned successfully!");</script>';
  }
 }
}
ob_clean();
header('Location:../staff/ticket-inbox.php');
echo '<script>alert("You do not have permission to assign this ticket to yourself!");</script>';
?>